...
void progonka(tovar store[], int needmas[])
{
	int ezzy;
	tovar* store_time = new tovar[needmas[1]];
	for (int i = 0; i < needmas[1]; i++)
	{
		store_time[i].section = store[i].section;
		store_time[i].name = store[i].name;
		store_time[i].price = store[i].price;
		store_time[i].vallue = store[i].vallue;
	}
	delete[] store;
	for (int i = 0; i < needmas[1]; i++)
	{
		for (int j = 0; j < needmas[1]; j++)
		{
			if (j != i)
			{
				if ((store[i].name == store[j].name) && (store[i].price == store[j].price))
				{
					store[i].vallue++;
					store[j].name = "0";
					ezzy++;
